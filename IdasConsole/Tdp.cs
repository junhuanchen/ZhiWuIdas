...
namespace Tdp
{
...

    {
        ...
        public TdpPack(byte[] data, EndPoint remoteEndPoint)
        {
            this.Data = data;
    ...
    public abstract class TdpServer   //#传udp输服务
    {
        ...
        protected abstract void PacketReceived(TdpPack buffer);
        protected abstract void PacketSent(TdpPack buffer, int bytesSent);

        // ServiceName
        String ServiceName;

        public TdpServer(String name, ushort port)
        {
            ...
            if (!shutdownFlag)
            {
                ...
                catch (SocketException se)
                {
                    ...
                    System.Diagnostics.EventLog.WriteEntry(ServiceName,
                        "A SocketException occurred in TdpServer.AsyncBeginReceive():\n\n" + se.Message,
        ...
        private void AsyncEndReceive(IAsyncResult iar)
        {
            ...
            if (!shutdownFlag)
            {
                ...
                TdpPack buffer = (TdpPack)iar.AsyncState;

                try
                {
                    // get the length of data actually read from the socket, store it with the
                    // buffer
                    buffer.Length = udpSocket.EndReceiveFrom(iar, ref buffer.RemoteEndPoint);
                ...
                catch (SocketException se)
                {
                    // something bad happened
                    System.Diagnostics.EventLog.WriteEntry(ServiceName,
                        "A SocketException occurred in TdpServer.AsyncEndReceive():\n\n" + se.Message,
                        System.Diagnostics.EventLogEntryType.Error);
        ...
        public void AsyncBeginSend(TdpPack pack)
        {
            ...
            if (!shutdownFlag)
            {
                ...
                {
                    System.Diagnostics.EventLog.WriteEntry(ServiceName,
                        "A SocketException occurred in TdpServer.AsyncBeginSend():\n\n" + se.Message,
                        System.Diagnostics.EventLogEntryType.Error);
                }
            }
        ...
        private void AsyncEndSend(IAsyncResult iar)
        {
            ...
            if (!shutdownFlag)
            {
                ...
                {
                    System.Diagnostics.EventLog.WriteEntry(ServiceName,
                        "A SocketException occurred in TdpServer.AsyncEndSend():\n\n" + se.Message,
                        System.Diagnostics.EventLogEntryType.Error);
                }
            }
